...
void ClapTrap::getInfo()
{
	...
	std::cout << YEL "Attac_Damage: " END << this->_attack_damage << std::endl;
	std::cout << YEL "Damage: " END << this->_take_damage << std::endl;
	std::cout << std::endl;
}
...
int ClapTrap::getTake_Damage() const
{
	return (this->_take_damage);
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (this->_health_points < 10 && this->_energy_points > 0)
	{
		...
		std::cout << GRE << this->getPerson_Name() << " recovered. + " << amount << " health points.\n\n" END;
	}
	else if (this->_energy_points < 0)
	{
		std::cout << RED << "The " << this->getPerson_Name() << " has no energy!\n\n" END;
